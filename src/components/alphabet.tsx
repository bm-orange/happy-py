    ...
    <div className={`min-h-screen px-4 py-8 bg-gradient-to-b from-blue-100 to-purple-100 ${hidden ? 'hidden' : '' }`}>
      <header className="container mx-auto mb-8">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl sm:text-4xl font-bold text-blue-600 flex items-center">
          ...
          <button className="flex items-center justify-center bg-orange-200 hover:bg-orange-300 text-orange-600 px-4 py-2 rounded-full text-xl sm:text-2xl" onClick={() => handleNav(1)}>
      ...
      <main className="container mx-auto">
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 justify-items-center gap-4">
            ...
            {data.consonants.map((v, i) => {
              return (
                <div key={i} className="w-12 h-12 sm:w-20 sm:h-20 bg-blue-100 text-blue-600 rounded-full cursor-pointer flex gap-2 items-center justify-center p-4" onClick={() => {
                  if (isPlaying) {
                    return
                  }
                  // e.stopPropagation()
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 justify-items-center gap-4">
            ...
            {data.vowels.map((v, i) => {
              return (
                <div key={i} className="w-12 h-12 sm:w-20 sm:h-20 bg-purple-100 text-purple-600  rounded-full cursor-pointer flex gap-2 items-center justify-center p-4" onClick={() => {
                  if (isPlaying) {
                    return
                  }
                  // e.stopPropagation()
