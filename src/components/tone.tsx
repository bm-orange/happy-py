...
export const Tone = ({ hidden, handleNav }: { hidden: boolean, handleNav: (n: number) => void }) => {
  ...
  const handleChangeSource = ({ newSource, char }: { newSource: string, char: string }) => {
    if (isPlaying) {
      return
    }
  ...
  return (
    ...
    <div className={`min-h-screen px-4 py-8 bg-gradient-to-b from-green-100 to-blue-100 ${hidden ? 'hidden' : ''}`}>
      <header className="container mx-auto mb-4 sm:mb-8">
        <div className="flex justify-between items-center">
          <h1 className="text-2xl sm:text-4xl font-bold text-green-600 flex items-center">
            <i className="w-5 h-5 sm:w-8 sm:h-8 mr-3">
          ...
          <button className="flex items-center gap-2 bg-orange-200 hover:bg-orange-300 text-orange-700 px-4 py-2 rounded-full sm:text-2xl text-xl" onClick={() => handleNav(1)}>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="tone-card bg-green-50 p-4 rounded-xl text-center cursor-pointer" onClick={() => {
              ...
              <div className="text-green-600 text-5xl font-bold mono-font">{tone.fourTone[0].char}</div>
            </div>
            <div className="tone-card bg-yellow-50 p-4 rounded-xl text-center cursor-pointer" onClick={() => {
              ...
              <div className="text-yellow-600 text-5xl font-bold mono-font">{tone.fourTone[1].char}</div>
            </div>
            <div className="tone-card bg-blue-50 p-4 rounded-xl text-center cursor-pointer" onClick={() => {
              ...
              <div className="text-blue-600 text-5xl font-bold mono-font">{tone.fourTone[2].char}</div>
            </div>
            <div className="tone-card bg-red-50 p-4 rounded-xl text-center cursor-pointer" onClick={() => {
              ...
              <div className="text-red-600 text-5xl font-bold mono-font">{tone.fourTone[3].char}</div>
            </div>
          </div>

          <div className="mt-6">
            <div className="text-6xl mb-2 sm:mb-4 text-center text-blue-600 mono-font">{tone.char}</div>
            <div className="flex flex-col items-center">
              <div className="mb-4 flex items-center justify-center space-x-4">
              <span className="text-xl font-bold mr-2">选择：</span>
                <select className="bg-white border-2 border-green-300 rounded-full px-2 py-2 text-xl text-center" disabled={isPlaying} defaultValue={toneType} onChange={(e) => {
                ...
                {tones[toneType as ToneType].map((v, i) => {
                  return (
                    <div key={i} className="w-20 h-20 bg-blue-100 text-blue-600 rounded-full cursor-pointer flex gap-2 items-center justify-center p-4" onClick={() => {
                      ...
                      <span className="mono-font text-2xl sm:text-4xl font-bold">{v.char}</span>
                    </div>
                  )
                })}
              </div>
      ...
      <Modal hidden={!isPlaying} char={char} />
    </div>
  )
}